...
function init() {
	...
	for(let i=0; i<tracks.length; i++){
		if(i%2==0){
			$('ul').append(`<li id="${i}"><div class="wrapper" style="background: white; height: 35px;"><div>${tracks[i].track}</div><div>${tracks[i].name}</div><div>${tracks[i].artist}</div><div>${tracks[i].duration}</div></div></li><hr/>`);
		}else{
			$('ul').append(`<li id="${i}"><div class="wrapper" style="background: #F5F7FF; height: 35px;"><div>${tracks[i].track}</div><div>${tracks[i].name}</div><div>${tracks[i].artist}</div><div>${tracks[i].duration}</div></div></li><hr/>`);
		}
